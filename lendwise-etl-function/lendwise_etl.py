...
def etl_pipeline(request):
    try:
        ...
        loan_repayments.columns = (
            loan_repayments.columns.str.replace(" ", "_")
        ...
        if "repayment_date" in loan_repayments.columns:
            loan_repayments["repayment_date"] = pd.to_datetime(
                loan_repayments["repayment_date"], format="%Y-%m-%d", errors="coerce"
            )
        ...
        # Clean the column names by replacing spaces with underscores and converting to lowercase
        loan_applications = loan_applications.rename(
        ...
        for date_col in date_columns:
            if date_col in loan_applications.columns:
                loan_applications[date_col] = pd.to_datetime(
                    loan_applications[date_col], format="%Y-%m-%d", errors="coerce"
                )
        ...
        missing_cols = [
            col
            for col in required_applicant_cols
            if col not in loan_applications.columns
        ]
        if missing_cols:
            ...
            available_cols = [
                col
                for col in required_applicant_cols
                if col in loan_applications.columns
            ]
            applicant_table = (
                loan_applications[available_cols]
                .drop_duplicates()
                .reset_index(drop=True)
            )
        else:
            applicant_table = (
                loan_applications[required_applicant_cols]
                .drop_duplicates()
                .reset_index(drop=True)
            )
        ...
        applicant_table["applicant_id"] = [
            "Ap" + str(i)
            for i in np.random.randint(100000, 999999, size=len(applicant_table))
        ]
        ...
        merge_cols = [
            col for col in required_applicant_cols if col in applicant_table.columns
        ]
        ...
        available_employment_cols = [
            col for col in employment_cols if col in employment_merged.columns
        ]
        if "applicant_id" not in available_employment_cols:
            available_employment_cols.append("applicant_id")

        employment_table = employment_merged[
            available_employment_cols
        ].drop_duplicates()
        ...
        employment_table["employment_id"] = [
            "Em" + str(i)
            for i in np.random.randint(100000, 999999, size=len(employment_table))
        ]

        # Reorder columns
        cols = ["applicant_id", "employment_id"] + [
            col
            for col in employment_table.columns
            if col not in ["applicant_id", "employment_id"]
        ]
        ...
        available_contact_cols = [
            col for col in contact_cols if col in contact_info_merged.columns
        ]
        contact_info_table = contact_info_merged[
            available_contact_cols
        ].drop_duplicates()

        np.random.seed(42)  # Seed set for reproducibility
        # Random contact_info IDs
        contact_info_table["contact_info_id"] = [
            "Ci" + str(i)
            for i in np.random.randint(100000, 999999, size=len(contact_info_table))
        ]

        # Reorder columns
        cols = ["contact_info_id", "applicant_id"] + [
            col
            for col in contact_info_table.columns
            if col not in ["contact_info_id", "applicant_id"]
        ]
        ...
        available_kin_cols = [
            col for col in kin_cols if col in next_of_kin_merged.columns
        ]
        next_of_kin_table = next_of_kin_merged[available_kin_cols].drop_duplicates()

        np.random.seed(42)  # Seed set for reproducibility
        # Random next of kin IDs
        next_of_kin_table["next_of_kin_id"] = [
            "Nk" + str(i)
            for i in np.random.randint(100000, 999999, size=len(next_of_kin_table))
        ]

        # Reorder columns
        cols = ["next_of_kin_id", "applicant_id"] + [
            col
            for col in next_of_kin_table.columns
            if col not in ["next_of_kin_id", "applicant_id"]
        ]
        ...
        available_fact_cols = [
            col for col in fact_cols if col in loan_application_fact_table.columns
        ]
        ...
        upload_to_gcs(
            loan_application_fact_table, "Cleaned_Data/loan_application_fact_table.csv"
        )
